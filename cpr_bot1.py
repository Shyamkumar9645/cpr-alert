...
try:
    ...
    print(f"Chart dependencies not installed: {e}. Charts will be disabled.")
    CHART_SUPPORT = False
...
class ChartGenerator:
    ...
    def create_cpr_chart(self, symbol: str, asset_name: str, candle_data: List[CandleData],
        ...
        """Create 5-minute candlestick chart with CPR levels and a clean white theme."""
        ...
        try:
            ...
            hlines = dict(
                ...
                colors=['#FF5252', '#367dfb', '#00E676'],  # Red, Blue, Green
                linestyle='--',
                linewidths=1.5,
                alpha=0.9
            )
            ...
            # Create the chart without the volume panel
            fig, axes = mpf.plot(
                df,
                type='candle',
                style=style,
                ...
                hlines=hlines,
                figsize=(12, 6),  # Adjust figsize as volume is removed
                returnfig=True,
                datetime_format='%H:%M',
                xrotation=0
            )
            ...
            # Add title and level info
            ax.text(0.02, 0.98, f'{asset_name} - 5min CPR', transform=ax.transAxes,
                   verticalalignment='top', horizontalalignment='left',
                   color='black', fontweight='bold', fontsize=12)

            level_text = f'R1: {cpr_levels.r1:.2f} | Pivot: {cpr_levels.pivot:.2f} | S1: {cpr_levels.s1:.2f}'
            ...
            ax.text(0.98, 0.93, f'{current_level.value} Alert @ {current_price:.2f}', transform=ax.transAxes,
                   verticalalignment='top', horizontalalignment='right',
                   color='#FF5722', fontweight='bold', fontsize=10) # Deep orange for alert

            # Save to bytes buffer
            buf = io.BytesIO()
            fig.savefig(buf, format='png', dpi=150, bbox_inches='tight', pad_inches=0.1, facecolor='white')
            buf.seek(0)

            plt.close(fig)

            logger.info(f"White theme chart generated successfully for {symbol}")
            return buf

        except Exception as e:
            logger.error(f"Error generating white theme chart for {symbol}: {e}")
            return None
    
    def get_chart_data(self, fyers_service, symbol: str, candle_count: int = 78, for_date: Optional[date] = None) -> List[CandleData]:
        """Get 5-minute candle data for a specific trading day."""
        try:
            if for_date:
                target_day = for_date
            else:
                target_day = DateHelper.get_previous_trading_day()

            # Define market hours in IST
            ist = pytz.timezone('Asia/Kolkata')
            market_start = ist.localize(datetime.combine(target_day, datetime.min.time()))
            market_end = ist.localize(datetime.combine(target_day, datetime.max.time()))

            # Convert to timestamps
            start_timestamp = int(market_start.timestamp())
            end_timestamp = int(market_end.timestamp())
            ...
            if response.get('s') == 'ok' and response.get('candles'):
                ...
                for candle in candles:
                    ...
                    candle_datetime = datetime.fromtimestamp(timestamp, tz=ist)
                    
                    # Ensure the candle is within market hours
                    if market_start.time() <= candle_datetime.time() <= market_end.time():
                ...
                # Return the last `candle_count` candles if more are returned
                return chart_candles[-candle_count:]
...
class ConfigManager:
    ...
    def load_config() -> Dict[str, Any]:
        ...
        # Initialize config with defaults or values from file_config
        config = {
            'fyers': file_config.get('fyers_credentials', {}),
            'telegram': file_config.get('telegram_credentials', {}),
            'assets': file_config.get('alert_settings', {}).get('symbols', [
                {'symbol': 'NSE:NIFTY50-INDEX', 'name': 'NIFTY 50', 'type': 'index', 'category': 'INDEX'},
                {'symbol': 'NSE:BANKNIFTY-INDEX', 'name': 'BANK NIFTY', 'type': 'index', 'category': 'INDEX'},
                {'symbol': 'NSE:RELIANCE-EQ', 'name': 'RELIANCE', 'type': 'stock', 'category': 'ENERGY'},
                # Add more default assets here if needed
            ]),
            'alert_settings': file_config.get('alert_settings', {}),
            'advanced_settings': file_config.get('advanced_settings', {}),
            'spam_prevention': file_config.get('spam_prevention', {}),
            'chart_settings': file_config.get('chart_settings', {}),
        }

        # Override with environment variables if set
        config['fyers']['app_id'] = os.getenv('FYERS_APP_ID', config['fyers'].get('app_id'))
        ...
        # Alert settings overrides
        alert_settings = config['alert_settings']
        alert_settings['check_interval_seconds'] = int(os.getenv('CHECK_INTERVAL_SECONDS', str(alert_settings.get('check_interval_seconds', 20))))
        alert_settings['tolerance_percent'] = float(os.getenv('TOLERANCE_PERCENT', str(alert_settings.get('tolerance_percent', 0.05))))
        alert_settings['cooldown_minutes'] = int(os.getenv('COOLDOWN_MINUTES', str(alert_settings.get('cooldown_minutes', 30))))
        alert_settings['preferred_resolution'] = os.getenv('PREFERRED_RESOLUTION', alert_settings.get('preferred_resolution', '1'))
        alert_settings['focus_on_key_levels'] = os.getenv('FOCUS_ON_KEY_LEVELS', str(alert_settings.get('focus_on_key_levels', True))).lower() == 'true'
        alert_settings['min_volume_threshold'] = int(os.getenv('MIN_VOLUME_THRESHOLD', str(alert_settings.get('min_volume_threshold', 5000))))
        alert_settings['enable_spam_prevention'] = os.getenv('ENABLE_SPAM_PREVENTION', str(alert_settings.get('enable_spam_prevention', True))).lower() == 'true'
        alert_settings['strict_level_crossing'] = os.getenv('STRICT_LEVEL_CROSSING', str(alert_settings.get('strict_level_crossing', True))).lower() == 'true'

        if 'market_hours' not in alert_settings: alert_settings['market_hours'] = {}
        market_hours = alert_settings['market_hours']
        market_hours['start'] = os.getenv('MARKET_START_TIME', market_hours.get('start', '09:15'))
        market_hours['end'] = os.getenv('MARKET_END_TIME', market_hours.get('end', '15:30'))
        market_hours['pre_market_start'] = os.getenv('PRE_MARKET_START', market_hours.get('pre_market_start', '09:00'))
        market_hours['post_market_end'] = os.getenv('POST_MARKET_END', market_hours.get('post_market_end', '15:45'))

        # Advanced settings overrides
        advanced_settings = config['advanced_settings']
        advanced_settings['enable_volume_analysis'] = os.getenv('ENABLE_VOLUME_ANALYSIS', str(advanced_settings.get('enable_volume_analysis', True))).lower() == 'true'
        advanced_settings['min_volume_threshold'] = int(os.getenv('MIN_VOLUME_THRESHOLD', str(advanced_settings.get('min_volume_threshold', 5000))))
        advanced_settings['enable_strength_filtering'] = os.getenv('ENABLE_STRENGTH_FILTERING', str(advanced_settings.get('enable_strength_filtering', True))).lower() == 'true'
        advanced_settings['min_touch_strength'] = float(os.getenv('MIN_TOUCH_STRENGTH', str(advanced_settings.get('min_touch_strength', 0.4))))
        advanced_settings['enable_trend_analysis'] = os.getenv('ENABLE_TREND_ANALYSIS', str(advanced_settings.get('enable_trend_analysis', False))).lower() == 'true'
        advanced_settings['enable_volatility_filter'] = os.getenv('ENABLE_VOLATILITY_FILTER', str(advanced_settings.get('enable_volatility_filter', True))).lower() == 'true'

        # Spam prevention overrides
        spam_prevention = config['spam_prevention']
        spam_prevention['min_candle_range_ratio'] = float(os.getenv('MIN_CANDLE_RANGE_RATIO', str(spam_prevention.get('min_candle_range_ratio', 0.3))))
        spam_prevention['max_alerts_per_hour'] = int(os.getenv('MAX_ALERTS_PER_HOUR', str(spam_prevention.get('max_alerts_per_hour', 12))))
        spam_prevention['require_volume_confirmation'] = os.getenv('REQUIRE_VOLUME_CONFIRMATION', str(spam_prevention.get('require_volume_confirmation', True))).lower() == 'true'
        spam_prevention['strict_crossing_validation'] = os.getenv('STRICT_CROSSING_VALIDATION', str(spam_prevention.get('strict_crossing_validation', True))).lower() == 'true'
        spam_prevention['crossing_tolerance_percent'] = float(os.getenv('CROSSING_TOLERANCE_PERCENT', str(spam_prevention.get('crossing_tolerance_percent', 0.02))))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "interactive":
            ...
            print("Usage: python cpr_bot1.py [interactive|test|config]")
    else:
        main()